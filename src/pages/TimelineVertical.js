...
import TimelinePointPage from './TimelinePointPage'
...
export default class TimelineVertical extends Component {
    ...
    render() {
        ...
        const point = list.map(item => {
            ...
            return(
                <TimelinePointPage previous={indx !== 0 ? parseInt(list[indx - 1].year) : null} id={item.title} distanceNeeded={isDistanceNeeded(indx)} index={item.id} box={this.state.box} data={this.state.data} handleHover={this.handleHover} handleHoverOut={this.handleHoverOut} year={item.year}/>  
